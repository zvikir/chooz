...
export async function POST(req: Request) {
  const { email, username, password, display_name, tag_slugs, gender, birthdate, location } = await req.json().catch(() => ({}))
  ...
  try {
    ...
    const inserted = await client.query(
      `INSERT INTO users (email, username, password_hash, display_name, gender, birthdate, location)
       VALUES ($1,$2,$3,$4,$5,$6,$7)
       RETURNING id, email, username, display_name, gender, birthdate, location`,
      [email, username, password_hash, display_name || null, gender, birthdate, location]
    )
